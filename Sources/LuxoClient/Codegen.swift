...
public final class LuxoCodegen {
    ...
    func generateClient(_ schema: LuxoSchema) -> String {
        ...
        var out = "// GENERATED BY LuxoCodegen. DO NOT EDIT.\n\n"
        out += "import Foundation\nimport LuxoClient\n\n"
        out += "public final class LuxoGeneratedClient: @unchecked Sendable {\n"
        out += "    private let transport: Transport\n"
        out += "    private let modeLock = NSLock()\n"
        out += "    private var binaryEnabled = false\n\n"
        out += "    private var useBinary: Bool {\n"
        out += "        modeLock.lock()\n"
        out += "        defer { modeLock.unlock() }\n"
        out += "        return binaryEnabled\n"
        out += "    }\n\n"
        ...
        for (_, api) in schema.apis.sorted(by: { $0.key < $1.key }) {
            ...
            if let params = api.params {
                ...
                paramSetup = "        var callParams: [String: LuxoValue] = [:]\n"
                for p in params {
                    let forceOptional = isPaginated && paginationNames.contains(p.name)
                    let nullable = p.nullable ?? false
                    let hasDefault = p.hasDefault ?? false
                    if nullable && hasDefault {
                        ...
                        paramSetup +=
                            "            if let value { callParams[\"\(p.name)\"] = try LuxoValue.encode(value) } else { callParams[\"\(p.name)\"] = .null }\n"
                        paramSetup += "        }\n"
                    } else if forceOptional || hasDefault {
                        paramSetup +=
                            "        if let \(p.name) { callParams[\"\(p.name)\"] = try LuxoValue.encode(\(p.name)) }\n"
                    } else if nullable {
                        paramSetup +=
                            "        if let \(p.name) { callParams[\"\(p.name)\"] = try LuxoValue.encode(\(p.name)) } else { callParams[\"\(p.name)\"] = .null }\n"
                    } else {
                        paramSetup +=
                            "        callParams[\"\(p.name)\"] = try LuxoValue.encode(\(p.name))\n"
                    }
                }
            }
            if hasStructuredReturn {
                ...
                paramSetup += "        if let select { callParams[\"$select\"] = .string(select) }\n"
            }
            if isPaginated {
                ...
                for name in ["page", "pageSize"] where !existingNames.contains(name) {
                    ...
                    paramSetup +=
                        "        if let \(name) { callParams[\"\(name)\"] = try LuxoValue.encode(\(name)) }\n"
                }
                if !paramList.isEmpty { paramList += ", " }
                ...
                paramSetup +=
                    "        if let filters { callParams[\"$filters\"] = .array(filters.map(\\.transportValue)) }\n"
                paramSetup +=
                    "        if let sorters { callParams[\"$sorters\"] = .array(sorters.map(\\.transportValue)) }\n"
            }

            if api.stream ?? false {
                let methodName = "subscribe" + api.name.prefix(1).uppercased() + api.name.dropFirst()
                if !paramList.isEmpty { paramList += ", " }
                paramList += "handler: @escaping @Sendable (\(swiftReturn)) -> Void"
                ...
                out += "            if self.useBinary {\n"
                if hasStructuredReturn {
                    if api.returnList ?? false {
                        out +=
                            "                guard let decoded = try? decodeColumnar\(returnType)(value) else { return }\n"
                        out += "                handler(decoded)\n"
                    } else {
                        out += "                var decoder = Decoder(value)\n"
                        out +=
                ...
                out += "            }\n"
                out +=
                    "            guard let decoded = try? JSONDecoder().decode(\(swiftReturn).self, from: value) else { return }\n"
                out += "            handler(decoded)\n"
                out += "        }\n"
                out += "    }\n\n"
                continue
            ...
            if returnType == "Void" {
                ...
                if api.paginated ?? false {
                    out += "            return try decodePaginated\(returnType)(result)\n"
                } else if api.returnList ?? false {
                    out += "            return try decodeColumnar\(returnType)(result)\n"
                } else {
                    out += "            var decoder = Decoder(result)\n"
            ...
            if returnType != "Void" {
                out += "        return try JSONDecoder().decode(\(swiftReturn).self, from: result)\n"
            }
            out += "    }\n\n"
        }
